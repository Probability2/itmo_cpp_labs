...
public:
  ...
  Take(Container my_container, const difference_type d)
  : my_container_(my_container)
  , shift_(d) {
  }

  Take& operator=(const Take& take) {
  ...
  iterator begin() {
    auto curr_end = my_container_.begin();
    size_type i = shift_;
    while (curr_end != my_container_.end() && i--) {
      ++curr_end;
    }

    return iterator(my_container_.begin(), my_container_.begin(), curr_end);
  }
  ...
  iterator end() {
    ...
    return iterator(curr_end, my_container_.begin(), curr_end);
  }
...
template<SimpleContainer container>
Take<container> take(const container& my_container, const difference_type shift) {
  return Take<container>(my_container, shift);
}

template<SimpleContainer container>
Take<container> operator|(container my_container, MyTake take_) {
  return Take<container>(my_container, take_.shift_);
}
